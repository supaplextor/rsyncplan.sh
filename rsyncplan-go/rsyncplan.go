...
func main() {
	...
	for _, mount := range mounts {
		if 0 != mount.Freq {
			fslabel := filepath.Base(mount.File)
			if fslabel == "/" {
				fslabel = "rootfs"
			}
			...
			//noisy log.Printf("%s from remote stdout: %s", __LINEETC__(), outputString)
			...
			for _, v := range ld {
				if len(v) != 0 {
					//noisy		log.Printf("%s appending %s (len(v)==%d", __LINEETC__(), "--link-dest="+v, len(v))
