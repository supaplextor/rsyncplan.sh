...
# NOTE: according to sources, this may be bwlimit i/o on the disk before 
# rsync reduces this, and it's also not related to network bandwidth.
# At least, I'm 50% sure...
...
go() {
	...
	links=`(ssh -n rbackup01 ls -1 $OFS ; echo sshec=$? >&2 ) |\
		tr " " "\n" |\
		grep -v $d |\
		sort -nr |\
		head -n 20 |\
	...
	rsync $ops $links $ifs rbackup01:/media/rbackup01/${me}/$ofs/$d/
}
